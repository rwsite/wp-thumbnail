...
trait Kama_Thumbnail_Admin_Part
{
    ...
    public function admin_options()
    {
        ...
        add_settings_field(
            'kt_options_field',
            '
			<p><a class="button" target="_blank" href="' . add_query_arg('kt_clear', 'clear_cache_stub') . '">' . __(
                'Clear nophoto Cache',
                'thumbnail'
            ) . '</a></p>
			
			<p><a class="button" target="_blank" href="' . add_query_arg('kt_clear', 'clear_cache') . '">' . __(
                'Clear all images',
                'thumbnail'
            ) . '</a></p>
			
			<p><a class="button" target="_blank" href="' . add_query_arg(
                'kt_clear',
                'delete_meta'
            ) . '" onclick="return confirm(\'' . __('Are You Sure?', 'thumbnail') . '\')">' . __(
                'Remove Releted Custom Fields',
                'thumbnail'
            ) . '</a></p>
        ...
        $elems = [
            ...
            'cache_folder' =>
                ...
                '<p class="description">' . __(
                    'Full path to the cache folder with 755 rights or above.',
                    'thumbnail'
                ) . '</p>',
            ...
            'meta_key' =>
				...
				<p class="description">' . __(
                    'Custom field key, where the thumb URL will be. Default:',
                    'thumbnail'
                ) . ' <code>' . $def_opt->meta_key . '</code></p>',

            'allow_hosts' =>
                '<textarea name="' . $opt_name . '[allow_hosts]" style="width:350px;height:45px;">' . esc_textarea(
                    $opt->allow_hosts
                ) . '</textarea>
				<p class="description">' . __(
                    'Hosts from which thumbs can be created. One per line: <i>sub.mysite.com</i>. Specify <code>any</code>, to use any hosts.',
                    'thumbnail'
                ) . '</p>',

            'quality' =>
                '<input type="text" name="' . $opt_name . '[quality]" value="' . $opt->quality . '" style="width:50px;">
				<p class="description" style="display:inline-block;">' . __(
                    'Quality of creating thumbs from 0 to 100. Default:',
                    'thumbnail'
                ) . ' <code>' . $def_opt->quality . '</code></p>',
            ...
            'no_stub' => '
				<label>
					...
					<input type="checkbox" name="' . $opt_name . '[no_stub]" value="1" ' . checked(
                    1,
                    @ $opt->no_stub,
                    0
                ) . '>
            ...
            'auto_clear' => '
				<label>
					...
					<input type="checkbox" name="' . $opt_name . '[auto_clear]" value="1" ' . checked(
                    1,
                    @ $opt->auto_clear,
                    0
                ) . '>
            ...
            'disable_http' => '
				<label>
					...
					<input type="checkbox" name="' . $opt_name . '[disable_http]" value="1" ' . checked(
                    1,
                    @ $opt->disable_http,
                    0
                ) . '>
            ...
            'rise_small' => '
				<label>
					...
					<input type="checkbox" name="' . $opt_name . '[rise_small]" value="1" ' . checked(
                    1,
                    @ $opt->rise_small,
                    0
                ) . '>
					' . __(
                    'Increase the thumbnail you create (width/height) if it is smaller than the specified size.',
                    'thumbnail'
                ) . '
				</label>',

            'use_in_content' => '
				<input type="text" name="' . $opt_name . '[use_in_content]" value="' . (isset($opt->use_in_content) ? esc_attr(
                    $opt->use_in_content
                ) : 'mini') . '">
				<p class="description">' . sprintf(
                    __(
                        'Find specified here class of IMG tag in content and make thumb from found image by it sizes. Leave this field empty to disable this function. Default: %s',
                        'thumbnail'
                    ),
                    '<code>mini</code>'
                ) . '</p>',
            ...
            'stop_creation_sec' => '
				<input type="number" step="0.5" name="' . $opt_name . '[stop_creation_sec]" value="' . (isset($opt->stop_creation_sec) ? esc_attr(
                    $opt->stop_creation_sec
                ) : 20) . '" style="width:4rem;"> ' . __('seconds', 'thumbnail') . '
				<p class="description">' . sprintf(
                    __(
                        'The maximum number of seconds since PHP started, after which thumbnails creation will be stopped. Must be less then %s (current PHP `max_execution_time`).',
                        'thumbnail'
                    ),
    ...
    static function def_options()
    {
        ...
        return apply_filters('kama_thumb_def_options', [
            'meta_key'          => 'photo_URL',
            // называние мета поля записи.
            'cache_folder'      => '',
            // полный путь до папки миниатюр.
            'cache_folder_url'  => '',
            // URL до папки миниатюр.
            'no_photo_url'      => '',
            // УРЛ на заглушку.
            'use_in_content'    => 'mini',
            // искать ли класс mini у картинок в тексте, чтобы изменить их размер.
            'no_stub'           => false,
            // не выводить картинку-заглушку.
            'auto_clear'        => false,
            // очищать ли кэш каждые Х дней.
            'auto_clear_days'   => 7,
            // каждые сколько дней очищать кэш.
            'rise_small'        => true,
            // увеличить создаваемую миниатюру (ширину/высоту), если её размер меньше указанного размера.
            'quality'           => 90,
            // качество создаваемых миниатюр.
            'allow_hosts'       => '',
            // доступные хосты, кроме родного, через запятую. 'any' - любые хосты.
            'stop_creation_sec' => 20,
            // макс кол-во секунд, с момента работы PHP, в которые миниатюры будут создаваться.
            'webp'              => false,
            // использвоать ли webp формат для создания миниатюр?
            'debug'             => 0,
            // режим дебаг (для разработчиков).
            'disable_http'      => false,
        ]);
    }
    ...
    {
        if (false === strpos($plugin_file, basename(KT_PATH))) {
            return $actions;
        }
