...
trait Kama_Thumbnail_Clear_Cache
{
        ...
        if (is_dir($cache_dir)) {
            ...
            if ($only_stub) {
                ...
                if (WP_DEBUG && current_user_can('manage_options')) {
                    self::show_message(
                        __('All nophoto thumbs was deleted from <b>Thumbnail</b> cache.', 'thumbnail'),
                        'notice-info'
                    );
                }
            } // delete all
    ...
    public function _delete_meta()
    {
        ...
        if (false !== $deleted) {
            self::show_message(
                sprintf(__('All custom fields <code>%s</code> was deleted.', 'thumbnail'), self::$opt->meta_key)
            );
        } else {
            self::show_message(
                sprintf(__('Couldn\'t delete <code>%s</code> custom fields', 'thumbnail'), self::$opt->meta_key)
            );
        }
        wp_cache_flush();
    }
    ...
    public function smart_clear($type = '')
    {
        ...
        if ($cleared || !$expire) {
            @ file_put_contents(
                $expire_file,
                time() + ($_stub ? DAY_IN_SECONDS : self::$opt->auto_clear_days * DAY_IN_SECONDS)
            );
        }
    }
